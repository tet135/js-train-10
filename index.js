...
function customAssign(...objects) {
  if (objects.length >= 2) {
    ...
    const bigObj = Object.assign(...objects);

    const newArray = [];
    for (const key in bigObj) { 
      if (typeof bigObj.key != "number") {
        newArray[key] = bigObj[key].toString();
      } 
    }
    return newArray;
...
function customObjectFromEntries(entries) {
  ...
  if (Array.isArray(entries)) {
    ...
    const stringEntries = entries.map(Fn);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (typeof obj1 === "object" && typeof obj2 === "object") {
    ...
    Object.freeze(obj1);
    return Object.isFrozen(obj1) ? obj1 : "Помилка";
  } else {
    return null;
  }
}
...
function getObjectValuesSum(obj) {
  if (typeof obj == "object") {
    ...
    const sum = num.reduce((total, nextElement) => total + nextElement);
    return sum;
  } else {
    return 0;
  }
...
console.log(convertArrayToObj(arr)); // Виведе `У масиві є дубльований ключ a`, {a: 4, b: 2, c: 3}
